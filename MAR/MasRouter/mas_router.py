...
class MasRouter(nn.Module):
    ...
    def forward(self, queries:List[str], tasks:List[Dict[str, str]], 
        ...
        # Role selection
        selected_roles_idx, role_log_probs, role_context, role_vae_loss, _ = self.role_allocation(tasks_role_emb_list, torch.concat([query_context, collab_context],dim=-1), agent_num_int) # N_q*agent_num, N_q*1，N_q*hidden_dim
        ...
        # LLM allocation
        selected_llms_idx, llm_log_probs, llm_vae_loss, _ = self.llm_router(llms_embedding, torch.concat([query_context, collab_context, role_context],dim=-1), agent_num_int, agent_num_float) # N_q*1，N_q*hidden_dim
...
class RoleAllocation(torch.nn.Module):
    ...
    def forward(self, roles_list:List[torch.Tensor], contexts:torch.Tensor, agent_num_int:torch.Tensor):
        ...
        summary_role_list = []
        per_position_log_probs = []  # per_position_log_probs[i][j] = scalar log_prob for query i, position j

        for i, roles in enumerate(roles_list): # each query
            selected_roles_idx.append([])
            per_position_log_probs.append([])
            ...
            for j in range(agent_num_int[i]): # each agent
                ...
                position_lp = torch.log(scores[torch.arange(scores.size(0)), selected_index]).unsqueeze(1)
                log_probs[i][0] = log_probs[i][0] + position_lp
                per_position_log_probs[i].append(position_lp.squeeze())
        ...
        return selected_roles_idx, log_probs, summary_role, vae_loss/len(roles_list), per_position_log_probs
...
class LLMRouter(torch.nn.Module):
    ...
    def forward(self, llms:torch.Tensor, contexts:torch.Tensor, agent_num_int:torch.Tensor, agent_num_float:torch.Tensor):
        ...
        per_position_log_probs:List[List[torch.Tensor]] = [[] for _ in range(contexts.size(0))]
        for i in range(1, self.max_agent+1):
            ...
            for j in range(contexts.size(0)):
                if agent_num_mask[j] > 0:
                    ...
                    per_position_log_probs[j].append(
                        torch.log(scores[j, selected_index[j]])
                    )
        ...
        return selected_llm_index, log_probs, vae_loss, per_position_log_probs
