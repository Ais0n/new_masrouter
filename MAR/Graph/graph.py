        ...
        for round in range(num_rounds):
            ...
            while zero_in_degree_queue:
                ...
                if self.collect_trace:
                    _node = self.nodes[current_node_id]
                    self.trace.append({
                        'step_id': len(self.trace),
                        'round': round,
                        'node_id': current_node_id,
                        'role': _node.role.role if hasattr(_node, 'role') and hasattr(_node.role, 'role') else '',
                        'llm_name': _node.llm.model_name if hasattr(_node, 'llm') else '',
                        'response': _node.outputs[-1] if _node.outputs else '',
                        'prompt': getattr(_node, '_last_prompt', None),
                        'spatial_predecessors': [p.id for p in _node.spatial_predecessors],
                        'temporal_predecessors': [p.id for p in _node.temporal_predecessors],
                    })
